...
public class CustomFActivity extends VenusAppBaseActivity implements View.OnClickListener {
    ...
    private java.util.Map<String, TextView> mMessageViews = new java.util.HashMap<>();
    ...
    // 多句子并发更新相关变量
    private java.util.Map<String, SentenceUpdateInfo> mActiveSentences = new java.util.HashMap<>();
    
    // 句子更新信息类
    private static class SentenceUpdateInfo {
        String msgId;
        int sentenceIndex;
        int appendCount;
        String baseTranscribe;
        String baseTranslate;
        
        SentenceUpdateInfo(String msgId, int sentenceIndex, String baseTranscribe, String baseTranslate) {
            this.msgId = msgId;
            this.sentenceIndex = sentenceIndex;
            this.appendCount = 0;
            this.baseTranscribe = baseTranscribe;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_custom_f_activity);
    ...
    protected void notifyVenusEnter() {
        VNCommon.setView(VNConstant.View.CUSTOM_F, null);
    }
    ...
    private void initConfig(boolean showHistoryText, boolean showOriginalAndTranslation, boolean useGlassesAudio) {
        ...
        // STEP 1:
        mVenusApp = VNConstant.View.CUSTOM_F;
    ...
    private void updateSttInfo(int actionType, String transcribe, String translate, String msgId) {
        // 发送到设备端
        ...
        sttInfo.setMsgId(msgId);
        VNCommon.updateSttInfo(mVenusApp, sttInfo, null);
        
        // 同时在UI上显示
        updateUIWithSttInfo(actionType, transcribe, translate, msgId);
    }
    
    /**
     * 在UI上更新语音转写信息
     */
    private void updateUIWithSttInfo(int actionType, String transcribe, String translate, String msgId) {
        runOnUiThread(() -> {
            TextView messageView;
            
            if (actionType == VNConstant.SttInfo.ActionType.NEW) {
                // 创建新的消息视图
                messageView = createMessageView(msgId);
                mMessagesContainer.addView(messageView);
                mMessageViews.put(msgId, messageView);
            } else {
                // 获取已有的消息视图
                messageView = mMessageViews.get(msgId);
                if (messageView == null) {
                    // 如果不存在，创建一个新的
                    messageView = createMessageView(msgId);
                    mMessagesContainer.addView(messageView);
            ...
            content.append("ID: ").append(msgId.substring(Math.max(0, msgId.length() - 4))).append("\n");
            content.append("原文: ").append(transcribe).append("\n");
            content.append("译文: ").append(translate);
            
            messageView.setText(content.toString());
     ...
     * 创建消息视图
     */
    private TextView createMessageView(String msgId) {
        ...
        // 根据msgId设置不同的背景颜色
        int colorHash = Math.abs(msgId.hashCode()) % 5;
        int backgroundColor;
        switch (colorHash) {
            case 0:
            ...
            case 2:
                ...
                break;
            case 3:
                backgroundColor = Color.parseColor("#9C27B0"); // 紫色
                break;
            default:
    ...
    private void checkTestCompletion() {
        // 如果没有活跃的句子且已经处理完所有句子，则测试完成
        if (mActiveSentences.isEmpty() && mCurrentSentenceIndex >= mTestTranscribes.length) {
            stopTest();
        }
    }
    ...
    private void sendNewSentence() {
        if (!mIsTesting || mCurrentSentenceIndex >= mTestTranscribes.length) {
            // 不再发送新句子，但不立即停止测试，等待所有句子完成
            return;
        }
        ...
        // 创建句子更新信息
        SentenceUpdateInfo sentenceInfo = new SentenceUpdateInfo(msgId, mCurrentSentenceIndex, currentTranscribe, currentTranslate);
        mActiveSentences.put(msgId, sentenceInfo);
        
        // 发送新句子
        updateSttInfo(VNConstant.SttInfo.ActionType.NEW, currentTranscribe, currentTranslate, msgId);

        // 延时后开始追加当前句子
        mHandler.postDelayed(() -> appendToSentence(msgId), 800);
        
        // 准备下一个句子（提前开始，实现穿插更新）
        mCurrentSentenceIndex++;
        if (mCurrentSentenceIndex < mTestTranscribes.length) {
            // 延时2秒后开始下一个句子，实现穿插效果
            mHandler.postDelayed(() -> sendNewSentence(), 2000);
        }
    }
     ...
     * 追加到指定句子
     */
    private void appendToSentence(String msgId) {
        if (!mIsTesting) {
            return;
        }
        
        SentenceUpdateInfo sentenceInfo = mActiveSentences.get(msgId);
        if (sentenceInfo == null) {
            return;
        }

        if (sentenceInfo.appendCount < 5) {
            // 追加对应的中文和英文内容
            String appendChinese = mAppendTextsChinese[sentenceInfo.appendCount % mAppendTextsChinese.length];
            String appendEnglish = mAppendTextsEnglish[sentenceInfo.appendCount % mAppendTextsEnglish.length];

            String updatedTranscribe = sentenceInfo.baseTranscribe + appendChinese;
            String updatedTranslate = sentenceInfo.baseTranslate + appendEnglish;
            
            // 更新基础内容
            sentenceInfo.baseTranscribe = updatedTranscribe;
            sentenceInfo.baseTranslate = updatedTranslate;

            // 发送更新
            updateSttInfo(VNConstant.SttInfo.ActionType.UPDATE, updatedTranscribe, updatedTranslate, msgId);

            sentenceInfo.appendCount++;

            // 继续追加，使用随机延时增加真实感
            int delay = 800 + (int)(Math.random() * 400); // 800-1200ms随机延时
            mHandler.postDelayed(() -> appendToSentence(msgId), delay);
        } else {
            // 追加完成，移除该句子
            mActiveSentences.remove(msgId);
