...
import com.jueyuantech.glasses.bean.PrompterCache;
...
import com.jueyuantech.venussdk.bean.VNPrompterInfo;
...
public class PrompterActivity extends VenusAppBaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContentTv.setTextSize(prompterConfig.getFontSize());
    ...
    private void loadFromUri(Uri uri) {
        ...
        File targetFile = new File(prompterFilesDir, fileName);
        InputStream is = null;
    ...
    private void loadContent(boolean formActivityResult) {
        if (prompterConfig.isAutoLoadHistory() || formActivityResult) { // 自动加载缓存开启 或 首次从外部加载
            if (null == prompterCache) { // 无缓存
                ...
                if (TextUtils.isEmpty(content)) {
                    ...
                    curPage = prompterCache.getCurPage();
                }
            }
        } else { // 未开启自动加载缓存
            promptContentStr = prompterInitTipStr;
    ...
    private String loadContentFromCache() {
        try {
            ...
            File prompterFile = new File(prompterFilesDir, prompterCache.getFileName());
    ...
    private void updatePromptContent() {
        ...
        try {
            totalPages = calculateTotalPages(totalLines, prompterConfig.getDisplayRows(), prompterConfig.isShowPrevious() ? 1 : 0);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateLines() {
        savePrompterCache(null, curPage);
        ...
        int endLine;
        try {
            int[] range = getPageRange(totalLines, prompterConfig.getDisplayRows(), prompterConfig.isShowPrevious() ? 1 : 0, curPage);
            startLine = range[0];
            endLine = range[1];
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static final String MMKV_KEY_PROMPTER_CACHE = "prompterCache";
    private PrompterCache prompterCache = null;

    private void loadPrompterCache() {
        prompterCache = (PrompterCache) MmkvUtil.getInstance().decodeParcelable(MMKV_KEY_PROMPTER_CACHE, PrompterCache.class);
    }

    private void savePrompterCache(String name, int page) {
        if (null == prompterCache) {
            prompterCache = new PrompterCache();
        }
        if (!TextUtils.isEmpty(name)) {
            prompterCache.setFileName(name);
        }
        prompterCache.setCurPage(page);
        MmkvUtil.getInstance().encodeParcelable(MMKV_KEY_PROMPTER_CACHE, prompterCache);
    }

    private void removePrompterCache() {
        MmkvUtil.getInstance().removeKey(MMKV_KEY_PROMPTER_CACHE);
        prompterCache = null;
    }
    ...
    private void loadPrompterConfig() {
        ...
        if (null == config) {
            ...
            prompterConfig.setDisplayRows(3);
            prompterConfig.setShowPrevious(false);
            prompterConfig.setFontSize(22);
            prompterConfig.setAutoLoadHistory(false);
        } else {
    ...
    private void showConfigDialog() {
        ...
        rowsTv.setText(String.valueOf(prompterConfig.getDisplayRows()));

        SwitchCompat showPreviousSwitch = dialogView.findViewById(R.id.switch_show_previous);
        showPreviousSwitch.setChecked(prompterConfig.isShowPrevious());
        ...
        autoLoadSwitch.setChecked(prompterConfig.isAutoLoadHistory());
        ...
        rowsIncreaseIv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (prompterConfig.getDisplayRows() >= 6) {
                    return;
                }
                prompterConfig.setDisplayRows(prompterConfig.getDisplayRows() + 1);
                savePrompterConfig();
                rowsTv.setText(String.valueOf(prompterConfig.getDisplayRows()));

                savePrompterCache(null, 0);
            }
        });
        ...
        rowsDecreaseIv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (prompterConfig.getDisplayRows() <= 2) {
                    return;
                }
                prompterConfig.setDisplayRows(prompterConfig.getDisplayRows() - 1);
                savePrompterConfig();
        ...
        showPreviousSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                prompterConfig.setShowPrevious(isChecked);
                savePrompterConfig();
        ...
        fontSizeIncrease.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                savePrompterConfig();
                setTextSizeDp(prompterConfig.getFontSize());
                fontSizeTv.setText(String.valueOf(prompterConfig.getFontSize()));
        ...
        autoLoadSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                prompterConfig.setAutoLoadHistory(isChecked);
                savePrompterConfig();
            }
        });
