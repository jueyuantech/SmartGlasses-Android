...
public class HomeMenuAdapter extends RecyclerView.Adapter<HomeMenuAdapter.HomeMenuViewHolder> {
    ...
    public HomeMenuAdapter(Context context, List<HomeMenu> homeMenus, int menuType) {
        mContext = context;
    ...
    public void onBindViewHolder(HomeMenuViewHolder holder, int position) {
        HomeMenu homeMenu = mHomeMenus.get(position);
    ...
    private void setDefault(HomeMenu homeMenu) {
        for (HomeMenu menu : mHomeMenus) {
    ...
    private int getMenuImage(String name, int defaultFlag) {
        ...
        } else if ("Asr".equals(name)) {
            if (1 == defaultFlag) {
                iconResourceId = R.mipmap.ic_venus_app_asr_selected;
            } else {
                iconResourceId = R.mipmap.ic_venus_app_asr_normal;
            }
        } else if ("Trans".equals(name)) {
            if (1 == defaultFlag) {
                iconResourceId = R.mipmap.ic_venus_app_trans_selected;
            } else {
                iconResourceId = R.mipmap.ic_venus_app_trans_normal;
            }
    ...
    interface OnRemovedListener {
        void onRemoved(HomeMenu homeMenu);
    }
